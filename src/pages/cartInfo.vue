...
export default {
  ...
  methods: {
    _initPageData() {
      const self = this;
      this.$axios("http://localhost:3000/goods/cartInfo").then(res => {
        this.cartInfo = res.data;
        self.$store.dispatch("setcartInfo", res.data);
      });
    },
    shopCheckAll(sid) {
    ...
    singleCheck(sid, pid) {
      ...
      let isChecked = this.cartInfo.shops[sid].products.every(
        (product, idx, arr) => {
    ...
    minusNum(sid, pid) {
      ...
      if (this.cartInfo.shops[sid].products[pid].num <= 1) {
        this.cartInfo.shops[sid].products[pid].num = 0;
      } else {
        this.cartInfo.shops[sid].products[pid].num--;
        this.$store.dispatch(
          "setcartInfo",
