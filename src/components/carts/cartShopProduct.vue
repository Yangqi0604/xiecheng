<template>
  <li style="position: relative">
    <div @swiperight="onSwipeRight" @swipeleft="onSwipeLeft">
      <div class="shop-info">
        <input
          type="checkbox"
          @change="singleCheck(sid,pid)"
    ...
    <transition name="swiper">
      <div @click="del(sid,pid)" class="item-del" v-if="isShow">删除</div>
    </transition>
  </li>
</template>
...
export default {
  ...
  computed: {
    num() {
      return this.$store.getters.this.cartInfo.shops[sid].products[pid].num;
    }
  }
};
</script>
